<script lang="ts">
  ...
  // const borderStyle = (border == 0) ? 0: `1px solid ${theme.borderColor}`;

  if (manual === "false") {
    fetch(`https://api.github.com/repos/${username}/${repository}`)
      .then((response) => response.json())
      .then((i) => {
        data.html_url = i.html_url;
        data.name = i.name;
        data.fork = i.fork;
        data.description = i.description;
        data.language = i.language;
        data.languageColor = githubLanguageColors[i.language];
        data.stars = i.stargazers_count;
        data.forks = i.forks;
      });
    }
  else {
    data.html_url = `https://github.com/${username}/${repository}`
    data.name = repository;
    data.description = description;
    data.language = language;
    data.languageColor = githubLanguageColors[data.language];
    data.stars = stars;
    data.forks = forks;
  }
</script>
...
<div id="container">
  <div style="color: {theme.color}; background-color: rgb(13, 17, 23)">
    <div style="margin-bottom: 8px">

      <span>
      <svg style="fill: {theme.color}" width="16" height="16">
        <path fill-rule="evenodd" d={repositoryIcon}></path>
      </svg>

      <span style="margin-left: 4px; font-weight: 600; color: {theme.linkColor}; width: 100%;">
        <a style="word-wrap: normal; text-decoration: none; color: inherit;" href="{data.html_url}">
          {data.name}
        </a>
      </span>
    </div>
    ...
    <div style="font-size: 12px;">
      <div style="margin-right: 16px;">
        <span style="width: 12px; height: 12px; border-radius: 100%; background-color: {data.languageColor}; display: inline-block; top: 1px; position: relative;"></span>
        <span>{data.language}</span>
      </div>

      <div style="display: flex; align-items: center;  margin-right: 16px;">
        <svg style="fill: {theme.color};" width="16" height="16">
          <path fill-rule="evenodd" d={starsIcon}></path>
        </svg>
        <span>&nbsp{data.stars}</span>
      </div>

      <div style="display: flex; align-items: center;">
        <svg style="fill: {theme.color};" width="16" height="16" >
          <path fill-rule="evenodd" d={forksIcon}></path>
        </svg>
        &nbsp;<span>{data.forks}</span>
      </div>
    </div>

    <div style="font-size: 12px; margin-bottom: 0; margin-top: 0px;">
      {data.description}
    </div>
